...
| `-AllowThirdPartyRepoMutation` | 他作リポジトリの node_modules ローカル cleanup を許可 |
| `-AllowUnknownRepoMutation` | 不明リポジトリの node_modules ローカル cleanup を許可 |
...
| `[脆弱性]` | 既知の CVE に脆弱な axios バージョンを使用中 | 手順 3（修復）に進む（1.15.0 以上にアップグレード） |
...
| CVE | 種類 | 影響バージョン | 修正版 |
|-----|------|--------------|--------|
| CVE-2025-27152 | SSRF + 認証情報漏洩（absolute URL） | 1.x: < 1.8.2 / 0.x: < 0.30.0 | 1.8.2 / 0.30.0 |
...
| axios バージョン | 自動 remediation | target |
|-----------------|-----------------|--------|
| 1.x 系 | 実行する | `1.15.0` (exact pin) |
| 0.x 系 | 実行しない | backport 未確定のため手順案内のみ |

**修復ポリシー（所有者別）：**

| アクション | 自作 (Mine) | 他作 (ThirdParty) | 不明 (Unknown) |
|-----------|------------|-------------------|----------------|
| RAT ファイル・レジストリの削除 | 実行する | 実行する | 実行する |
| IOC パッケージの削除 | 実行する | report-only ※ | report-only ※ |
| axios の lockfile 更新 | 実行する | report-only ※ | report-only ※ |
| クリーン再構築 (npm ci) | 実行する | report-only ※ | report-only ※ |
| 署名検証 (npm audit signatures) | 実行する | ― | ― |
| npm キャッシュのクリア | 実行する | 実行する | 実行する |

※ `-AllowThirdPartyRepoMutation` / `-AllowUnknownRepoMutation` を指定すると、node_modules 配下のローカル cleanup のみ許可されます。package.json / lockfile は変更しません。

**自作リポジトリの修復は 3 段階で行われます：**

1. **Phase A (MetadataOnly):** `npm install axios@1.15.0 --save-exact --package-lock-only --ignore-scripts` で lockfile と package.json の exact pin だけを更新
2. **Phase B (Reinstall):** `node_modules` を削除して `npm ci --ignore-scripts` でクリーン再構築
3. **Phase C (Verify):** `npm audit signatures` で npm registry の署名を検証

この分離により、Phase A の時点で lockfile diff と package.json diff を確認でき、実体の再構築を切り離せます。

他作/不明リポジトリではデフォルトで repo tree を変更しません。`ManualActions.txt` に upstream maintainer への報告手順を含む手動対応案内が生成されます。
...
**修復ポリシー：**
- 1.x 系: 自動 remediation target は `1.15.0`（exact pin）。lockfile 更新 → クリーン再構築 → 署名検証の 3 段階
- 0.x 系: backport 未確定のため自動 remediation なし。手順案内のみ
- 自作リポジトリ (Mine): 3 段階の自動修復を実行
- 他作/不明リポジトリ: デフォルトで report-only（`-AllowThirdPartyRepoMutation` / `-AllowUnknownRepoMutation` で opt-in 可能）
